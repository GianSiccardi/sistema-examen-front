...
eliminarPregunta(preguntaId:any){
  Swal.fire({
    ...
    text:"Estas seguro que quieres eliminar la pregunta?",
    icon:"warning",
...
if(resultado.isConfirmed){
  this.preguntasService.eliminarPregunta(preguntaId).subscribe(
    (data)=>{
     ...
     this.preguntas=this.preguntas.filter((pregunta:any)=>pregunta.preguntaId!=preguntaId)//colocamos todas las pregunta menos la q eliminamos

    },
    (error)=>{
      this.snack.open("Error al eliminar pregunta","",{
        duration:3000
      })
    }
  )
